...
def fetch_manual_journals(access_token, xero_tenant_id):
    ...
    while True:
        ...
        params = {
            "where": f'UpdatedDateUTC >= {updated_str} AND Status=="POSTED"',
            "page": page,
            "pageSize": 1000,
        }
...
def parse_manual_journal_lines(journals, client_name):
    ...
    for journal in journals:
        ...
        for line in journal.get("JournalLines", []):
            if client_name == "FUTUREYOU_RECRUITMENT" and str(line.get("AccountCode")) not in account_code_mapping: continue
