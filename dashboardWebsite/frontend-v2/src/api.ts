...
export const triggerWorkflow = async (
  ...
  try {
    ...
    return {
      success: false,
      message: String(response.data.error ?? "Unknown error"),
    };
  } catch (error) {
    if (axios.isAxiosError(error)) {
      ...
      if (backendData?.error)
        return { success: false, message: String(backendData.error) };
      return {
        success: false,
...
export const fetchFileInfo = async (repoPath: string): Promise<FileInfo> => {
  const response = await fetch(
    `${API_BASE_URL}/file-info?path=${encodeURIComponent(repoPath)}`,
  );
