...
import {
  FileText, Database, RefreshCw, CreditCard, FileCheck, Mail, Zap,
  ChevronRight, Clock, AlertCircle, CheckCircle2, XCircle, MinusCircle,
...
import {
  triggerWorkflow, uploadFile, authenticateUser,
  fetchRunStatus, type AuthUser, type WorkflowStep, type RunStatus,
} from '@/api'
...
const CLIENTS: ClientConfig[] = [
    ...
    scripts: [
      {
        ...
        description: 'Generates an Aged Trial Balance report that includes both FutureYou Recruitment and Contracting Invoices.',
      ...
      },
      {
        ...
        description: 'Updates the Invoice Revenue BigQuery Database by syncing invoices changed within the last 24 hours.',
      ...
      },
      {
        ...
        description: 'Updates the ATB BigQuery Database, automatically refreshing the Master ATB file with the latest data.',
    ...
    scripts: [
      {
        ...
        description: 'Allocates supplier DPs to any approved bills in Xero. DP list is current as of the first day of the month.',
      ...
      },
      {
        ...
        description: 'Approves draft invoices and stock journals in Xero for H2coco. Excludes Costco Australia invoices.',
  ...
  },
  {
    ...
    scripts: [
      {
        name: 'Draft Invoice Approver',
        description: 'Approves draft invoices and stock journals in Xero for FlightRisk, with Unleashed sales order validation.',
      ...
      },
      {
        ...
        description: 'Allocates customer prepayments to approved AR invoices in Xero and sends a summary report to your email.',
    ...
    scripts: [
      {
        ...
        description: 'Sends a test email to verify SMTP settings and connectivity.',
      ...
      },
      {
        ...
        description: 'Tests the API connection with a sample request to verify the backend is reachable.',
...
function StepIcon({ step }: { step: WorkflowStep }) {
  if (step.status === 'completed') {
    if (step.conclusion === 'success')  return <CheckCircle2 className="h-4 w-4 text-emerald-500 shrink-0" />
    if (step.conclusion === 'failure')  return <XCircle      className="h-4 w-4 text-red-500 shrink-0" />
    if (step.conclusion === 'skipped')  return <MinusCircle  className="h-4 w-4 text-muted-foreground shrink-0" />
  }
  if (step.status === 'in_progress') return <Loader2 className="h-4 w-4 text-primary animate-spin shrink-0" />
  return <Circle className="h-4 w-4 text-muted-foreground/40 shrink-0" />
}

function trackingLabel(status: RunStatus['status'], conclusion: RunStatus['conclusion']): string {
...
const formatEstimate = (s: number) => s >= 60 ? `~${Math.round(s / 60)}m` : `~${s}s`
...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (dialogStep !== 'tracking' || !trackingWorkflowFile || !dispatchedAt) return
        ...
        if (status.status === 'completed') {
          ...
          setTimeout(() => {
            ...
            if (status.conclusion === 'success') {
              toast.success(`${selectedScript?.name ?? 'Script'} completed successfully!`)
            } else {
              toast.error(`${selectedScript?.name ?? 'Script'} ${status.conclusion ?? 'failed'}.`)
            }
    ...
    const timeout = setTimeout(() => {
      ...
      closeDialog()
      toast.error('Workflow is taking longer than expected. Check GitHub Actions.')
    }, 15 * 60 * 1000)
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (selectedScript.requiresFileUpload && uploadedFile) {
        ...
        if (!uploadResponse.success) { setStatusMessage(uploadResponse.message); return }
      }
    ...
    <div className="flex h-screen bg-background">
      ...
      <main className="flex-1 overflow-auto">
        <div className="border-b border-border bg-card px-8 py-6">
          <h1 className="text-2xl font-bold" style={{ color: '#151f4a' }}>Dashboard</h1>
            ...
            <Card key={client.clientName}>
              <CardHeader>
                <div className="flex items-center gap-2">
                  <Badge className={client.color} variant="outline">{client.clientName}</Badge>
                </div>
              </CardHeader>
              <CardContent>
                  ...
                  {client.scripts.map((script) => {
                    ...
                    return (
                      <button
                        ...
                        style={{ ['--tw-hover-border-color' as string]: '#38bdf8' }}
                        onMouseEnter={e => (e.currentTarget.style.borderColor = '#38bdf8')}
                        onMouseLeave={e => (e.currentTarget.style.borderColor = '')}
                      >
                        <div className="flex items-start justify-between">
                          <div className="p-2 rounded-lg" style={{ backgroundColor: '#151f4a15', color: '#151f4a' }}>
                            <Icon className="h-4 w-4" />
                          </div>
                          <ChevronRight className="h-4 w-4 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity mt-0.5" />
                        </div>
                        <div>
                          <p className="font-medium text-sm text-foreground leading-tight">{script.name}</p>
                          <p className="text-xs text-muted-foreground mt-1 line-clamp-2 leading-relaxed">{script.description}</p>
                        </div>
            ...
            <>
              <DialogHeader>
                <div className="flex items-center gap-2 mb-1">
                  <Badge className={selectedClientColor} variant="outline">{selectedClient}</Badge>
                </div>
              ...
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-muted text-sm text-muted-foreground">
                ...
                <span>Estimated run time: {formatEstimate(selectedScript.estimatedTime)}</span>
              </div>
              <DialogFooter>
          ...
          {dialogStep === 'auth' && selectedScript && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  Enter your credentials to run <strong>{selectedScript.name}</strong>.
                </DialogDescription>
              </DialogHeader>
              ...
              <DialogFooter>
                <Button variant="outline" onClick={() => setDialogStep('info')} disabled={isSubmitting}>Back</Button>
          ...
          {dialogStep === 'tracking' && selectedScript && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  {runStatus
                    ? trackingLabel(runStatus.status, runStatus.conclusion ?? null)
              ...
              <div className="space-y-3">
                  ...
                  <div className="rounded-lg border border-border divide-y divide-border overflow-hidden">
                    {runStatus.steps.map((step, i) => (
                      <div key={i} className="flex items-center gap-3 px-3 py-2.5">
                        ...
                        <span className={cn(
                          'text-sm flex-1',
                          step.status === 'queued' ? 'text-muted-foreground' : 'text-foreground'
                        )}>
                          {step.name}
                        </span>
                ...
                <p className="text-xs text-muted-foreground">
                  The workflow will continue running on GitHub even if you close this window.
                </p>
              </div>
            </>
          )}
